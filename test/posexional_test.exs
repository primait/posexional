defmodule PosexionalTest do
  ...
  test "different separator" do
    row = Row.new(:test, [Field.Value.new(:code, 8, ?0, :right)])
    file = File.new([row], "\n\r")
  ...
  test "read a file and outputs a keyword list" do
    row = Row.new(:test, [Field.Value.new(:code, 4, ?0, :right)], row_guesser: :always)
    file = File.new([row])
  ...
  test "read a file and outputs a keyword list with progressive number field" do
    fields = [
      Field.Value.new(:code, 4, ?0, :right),
      Field.ProgressiveNumber.new(3, ?0)
    ]
  ...
  test "fixed value field" do
    fields = [Field.Value.new(:code, 4, ?0, :right), Field.FixedValue.new("test")]
