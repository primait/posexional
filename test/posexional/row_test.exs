defmodule Posexional.RowTest do
  ...
  test "fields_from to copy fields from another row" do
    from_row =
      Row.new(:from, [Field.Value.new(:v1, 5), Field.Value.new(:v2, 5), Field.Value.new(:v3, 5)])
  ...
  test "an unkonwn row do not match" do
    assert [
             ...
             Posexional.RowTest.FileModule.read(
               "test|A       |00001|-----\nnono|A       |00002|-----"
             )
  end

  test "import field from another row module yields a row struct with the same fields" do
    assert Posexional.RowTest.OtherRowModule.get_row().fields ===
             Posexional.RowTest.RowModule.get_row().fields
  end

  test "file module with imported row works" do
    assert [
             {Posexional.RowTest.OtherRowModule,
              [fixed_value: "test", a: "A       ", progressive: 1]},
             "nono|A       |00002|-----"
           ] ===
             Posexional.RowTest.OtherFileModule.read(
               "test|A       |00001|-----\nnono|A       |00002|-----"
             )
  end
  ...
  test "default is ignored if value is provided" do
    assert "defaultrow|B       " ===
             Posexional.RowTest.DefaultFileModule.write(with_defaults: [a: "B"])
  end
